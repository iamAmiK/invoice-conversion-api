...
exports.convertFlexibleJsonToUblXml = void 0;
...
const findFieldValue = (obj, searchFields) => {
                ...
                if (typeof value === 'object' && value !== null) {
                    ...
                    if (Array.isArray(value) && value.length > 0) {
                        ...
                        if (typeof firstItem === 'object') {
                            ...
                            if (firstItem.IdentifierContent !== undefined)
                                return firstItem.IdentifierContent;
                            if (firstItem.TextContent !== undefined)
                                return firstItem.TextContent;
                            if (firstItem.AmountContent !== undefined)
                                return firstItem.AmountContent;
                            if (firstItem.DateContent !== undefined)
                                return firstItem.DateContent;
                            if (firstItem.NumericContent !== undefined)
                                return firstItem.NumericContent;
                            if (firstItem.CodeContent !== undefined)
                                return firstItem.CodeContent;
                        }
                        return firstItem;
                    }
                }
...
const extractAttributes = (obj, searchFields) => {
    const attributes = {};
    if (typeof obj !== 'object' || obj === null) {
        return attributes;
    }
...
const buildPartyStructure = (partyData, partyType) => {
    const party = {};
    // Endpoint ID
    ...
    if (taxData) {
        ...
        if (companyId)
            taxScheme['cbc:CompanyID'] = [companyId];
...
const buildLineItems = (lineItemsData) => {
    ...
    return lineItemsData.map((item, index) => {
        ...
        if (accountingCost)
            line['cbc:AccountingCost'] = [accountingCost];
        ...
        if (itemData) {
            ...
            if (properties) {
                ...
                itemObj['cac:AdditionalItemProperty'] = properties.map((prop) => ({
...
const buildDocumentReference = (docData) => {
    ...
    if (docType)
        docRef['cbc:DocumentType'] = [docType];
    return [docRef];
};
// Build document references structure
const buildDocumentReferences = (docsData) => {
...
const buildPaymentTermsStructure = (termsData) => {
    const terms = {};
...
const buildAllowanceChargeStructure = (chargeData) => {
    if (!Array.isArray(chargeData)) {
        chargeData = [chargeData];
    }
    return chargeData.map((charge) => {
        ...
        if (reason)
            allowanceCharge['cbc:AllowanceChargeReason'] = [reason];
        ...
        if (multiplier)
            allowanceCharge['cbc:MultiplierFactorNumeric'] = [multiplier];
...
const buildTaxTotalStructure = (taxData) => {
    const taxTotal = {};
    ...
    if (subtotals) {
        ...
        taxTotal['cac:TaxSubtotal'] = subtotals.map((subtotal) => {
            const taxSubtotal = {};
            ...
            if (categoryData) {
                ...
                if (exemptionCode)
                    category['cbc:TaxExemptionReasonCode'] = [exemptionCode];
                const exemptionReason = findFieldValue(categoryData, ['taxExemptionReason', 'exemptionReason']);
                if (exemptionReason)
                    category['cbc:TaxExemptionReason'] = [exemptionReason];
...
const convertFlexibleJsonToUblXml = async (invoiceData) => {
    try {
        if (!invoiceData) {
...
exports.convertFlexibleJsonToUblXml = convertFlexibleJsonToUblXml;
